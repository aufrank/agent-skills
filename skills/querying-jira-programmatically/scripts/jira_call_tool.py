#!/usr/bin/env python3
"""
Generic Jira mcpc tool caller with cached tools-list/tools-get under .mcpc-skill-caches/.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Call any Jira MCP tool with cached schemas.")
    parser.add_argument("--tool", required=True, help="Tool name to call (e.g., search, searchJiraIssuesUsingJql).")
    parser.add_argument(
        "--arg",
        action="append",
        default=[],
    ...
    parser.add_argument("--output", default=None, help="Output JSON path (default: results/<tool>.json).")
...
def cache_tools_list(mcpc_cmd: List[str], profile: str, session: str, refresh: bool, path: Path) -> List[Dict[str, Any]]:
...
def _extract_tool_names(tools_payload: Any) -> List[str]:
    if isinstance(tools_payload, list):
        return [t.get("name") for t in tools_payload if isinstance(t, dict) and t.get("name")]
...
def main() -> None:
    ...
    available = set(_extract_tool_names(tools_list))
