#!/usr/bin/env python3
"""
Summarize what each team member is working on via JQL search.
Builds a JQL from project/team or uses a provided JQL, and groups issues by assignee.
"""
...
def build_jql(args: argparse.Namespace) -> str:
    ...
    if args.team:
        clauses.append(f'assignee in membersOf("{args.team}")')
    if not args.include_done:
        clauses.append('statusCategory != Done')
    return " AND ".join(clauses)


def summarize_by_assignee(issues: List[Dict[str, Any]]) -> str:
    ...
    for issue in issues:
        ...
        if isinstance(assignee_obj, dict):
            assignee = assignee_obj.get("displayName") or assignee_obj.get("name") or assignee_obj.get("emailAddress") or ""
        ...
        buckets[assignee].append(f"{key} [{status}] {summary}")

    lines: List[str] = []
    for assignee, items in sorted(buckets.items()):
        lines.append(f"{assignee} — {len(items)} issue(s)")
        for item in items:
            lines.append(f"  - {item}")
    return "\n".join(lines)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Group Jira issues by assignee for a team/project.")
    parser.add_argument("--project", help="Project key (e.g., MLPLAT). Required unless --jql provided.")
    parser.add_argument("--team", help="Jira group/team name used with membersOf().")
    parser.add_argument("--jql", help="Custom JQL. If set, overrides project/team.")
    ...
    parser.add_argument("--output", default=None, help="Path to write parsed payload (default results/jira.team_workload.json).")
    parser.add_argument("--summary-output", default=None, help="Path to write text summary (default results/jira.team_workload.txt).")
    ...
    output_path = Path(args.output) if args.output else roots["results_root"] / "jira.team_workload.json"
    summary_path = Path(args.summary_output) if args.summary_output else roots["results_root"] / "jira.team_workload.txt"
    ...
    summary_text = summarize_by_assignee(issues)
    print(summary_text)
