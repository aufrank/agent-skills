#!/usr/bin/env python3
"""
Find open, unassigned issues for a project (or custom JQL).
"""
...
def build_jql(args: argparse.Namespace) -> str:
    ...
    if not args.project:
        ...
        sys.exit(1)
    clauses = [f"project = {args.project}", "assignee is EMPTY"]
    if not args.include_done:
        clauses.append("statusCategory != Done")
    return " AND ".join(clauses)


def _print_summary(issues: List[Dict[str, Any]]) -> None:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Find unassigned issues via JQL.")
    parser.add_argument("--project", help="Project key (e.g., MLPLAT). Required unless --jql provided.")
    parser.add_argument("--jql", help="Custom JQL. If set, overrides project.")
    ...
    parser.add_argument("--output", default=None, help="Path to write parsed payload (default results/jira.unassigned.json).")
    parser.add_argument("--raw-output", default=None, help="Optional path to write raw mcpc response.")
    parser.add_argument("--no-output", action="store_true", help="Skip writing parsed output.")
    ...
    output_path = Path(args.output) if args.output else roots["results_root"] / "jira.unassigned.json"
    ...
    for path in [output_path, raw_output_path]:
